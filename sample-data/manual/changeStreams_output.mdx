...
without the prior complexity and risk of manually tailing the <Ref url="oplog">{refs["oplog"].title}</Ref>.
...
Change streams are available for <Ref url="replication">{refs["replication"].title}</Ref> and
<Ref url="sharding-background">{refs["sharding-background"].title}</Ref>:

* **Storage Engine.**

  The replica sets and sharded clusters must use the <Ref url="storage-wiredtiger">{refs["storage-wiredtiger"].title}</Ref> storage engine. Change streams can also be used
  on deployments that employ MongoDB's
  <Ref url="encrypted-storage-engine">{refs["encrypted-storage-engine"].title}</Ref> feature.

* **Replica Set Protocol Version.**

  The replica sets and sharded clusters must use replica set protocol
  version 1 (<Ref url="rsconf.protocolVersion">{refs["rsconf.protocolVersion"].title}</Ref>).
...
<Ref url="connections-read-preference">{refs["connections-read-preference"].title}</Ref>. If the driver cannot find
a node with the correct read preference, it throws an exception.

For more information, see <Ref url="mongodb-uri">{refs["mongodb-uri"].title}</Ref>.
...
<ListTable header-rows={{"$numberInt":"1"}} widths="15 85">
  ...
  * * A collection
    ...
    * You can open a change stream cursor for a single collection
      ...
      also the <Ref url="https://www.mongodb.com/docs/mongodb-shell/#mongodb-binary-bin.mongosh">{refs["https://www.mongodb.com/docs/mongodb-shell/#mongodb-binary-bin.mongosh"].title}</Ref> method
      <Ref url="db.collection.watch">{refs["db.collection.watch"].title}</Ref>.
  ...
  * * A database
    ...
    * You can open a change stream cursor for a single database (excluding
      ...
      documentation. See also the <Ref url="https://www.mongodb.com/docs/mongodb-shell/#mongodb-binary-bin.mongosh">{refs["https://www.mongodb.com/docs/mongodb-shell/#mongodb-binary-bin.mongosh"].title}</Ref> method
      <Ref url="db.watch">{refs["db.watch"].title}</Ref>.
  ...
  * * A deployment
    ...
    * You can open a change stream cursor for a deployment (either a replica
      ...
      For the MongoDB driver method, refer to your driver
      documentation. See also the <Ref url="https://www.mongodb.com/docs/mongodb-shell/#mongodb-binary-bin.mongosh">{refs["https://www.mongodb.com/docs/mongodb-shell/#mongodb-binary-bin.mongosh"].title}</Ref> method
      <Ref url="Mongo.watch">{refs["Mongo.watch"].title}</Ref>.
</ListTable>
...
<Ref url="connection-pool-overview">{refs["connection-pool-overview"].title}</Ref>, you may
experience notification latency. Each change stream uses a connection
and a <Ref url="manual-reference-commands-getMore">{refs["manual-reference-commands-getMore"].title}</Ref>
...
number of opened change streams. For details see the <Ref url="maxpoolsize-cp-setting">{refs["maxpoolsize-cp-setting"].title}</Ref> setting.

### Sharded Cluster Considerations

When a change stream is opened on a sharded cluster:

* The <Ref url="bin.mongos">{refs["bin.mongos"].title}</Ref> creates individual change streams on **each
...
For best performance, limit the use of <Ref url="pipe.$lookup">{refs["pipe.$lookup"].title}</Ref> queries in
change streams.
...
* For a sharded cluster, you must issue the open change stream
  operation from the <Ref url="bin.mongos">{refs["bin.mongos"].title}</Ref>.
...
To retrieve the <Ref url="change-stream-output">{refs["change-stream-output"].title}</Ref> from
the cursor, iterate the change stream cursor. For information on the
change stream event, see <Ref url="change-stream-output">{refs["change-stream-output"].title}</Ref>.
...
<Tabs tabset="drivers">
  ...
  <Tab tabid="java-sync">
    ...
    The `pipeline` list includes a single <Ref url="pipe.$match">{refs["pipe.$match"].title}</Ref> stage that
    ...
    * `operationType` value is `delete`
    ...
    Passing the `pipeline` to the <Ref url="db.collection.watch">{refs["db.collection.watch"].title}</Ref> method directs the
...
<Tip>
  The <Ref url="change-stream-event-id">{refs["change-stream-event-id"].title}</Ref> field of the change stream
  event document act as the <Ref url="change-stream-resume">{refs["change-stream-resume"].title}</Ref>. Do not use the pipeline to modify or remove
  the change stream event's `_id` field.

  <_4_2ChangesChangeStreamModificationError />

  See <Ref url="change-stream-output">{refs["change-stream-output"].title}</Ref> for more information on the change stream
...
<Tabs tabset="drivers">
  ...
  <Tab tabid="csharp">
    ...
    <Ref url="db.collection.watch">{refs["db.collection.watch"].title}</Ref> method.
  ...
  <Tab tabid="php">
    ...
    to the <Ref url="db.watch">{refs["db.watch"].title}</Ref> method.
  ...
  <Tab tabid="ruby">
    ...
    To return the most current majority-committed version of the updated
    document, pass `full_document: 'updateLookup'` to the
    <Ref url="db.watch">{refs["db.watch"].title}</Ref> method.
...
<Note>
  ...
  * If the update changes the values for at least one of the fields in
  ...
  See <Ref url="change-stream-output">{refs["change-stream-output"].title}</Ref> for more information on the change
...
<Ref url="change-stream-resume-after">{refs["change-stream-resume-after"].title}</Ref> or
<Ref url="change-stream-start-after">{refs["change-stream-start-after"].title}</Ref> when opening the cursor.
...
See <Ref url="change-stream-resume-token">{refs["change-stream-resume-token"].title}</Ref> for more information on the resume token.
...
<Ref url="change-stream-resume-after">{refs["change-stream-resume-after"].title}</Ref>, `startAfter` can
resume notifications after an <Ref url="change-event-invalidate">{refs["change-event-invalidate"].title}</Ref>
...
<ListTable header-rows={{"$numberInt":"1"}} stub-columns={{"$numberInt":"1"}} widths="20 80">
  ...
  * * <Ref url="change-event-resume-token">{refs["change-event-resume-token"].title}</Ref>

    * Each change event notification includes a resume token
      on the `_id` field.

  * * <Ref url="aggregate-resume-token">{refs["aggregate-resume-token"].title}</Ref>

    * The <Ref url="pipe.$changeStream">{refs["pipe.$changeStream"].title}</Ref> aggregation stage includes
      a resume token on the `cursor.postBatchResumeToken` field.

      This field only appears when using the <Ref url="dbcmd.aggregate">{refs["dbcmd.aggregate"].title}</Ref>
      command.

  * * <Ref url="getMore-resume-token">{refs["getMore-resume-token"].title}</Ref>

    * The <Ref url="dbcmd.getMore">{refs["dbcmd.getMore"].title}</Ref> command includes a resume token on the
...
When using the <Ref url="dbcmd.aggregate">{refs["dbcmd.aggregate"].title}</Ref> command, the <Ref url="pipe.$changeStream">{refs["pipe.$changeStream"].title}</Ref>
...
The <Ref url="dbcmd.getMore">{refs["dbcmd.getMore"].title}</Ref> command also includes a resume token on the
`cursor.postBatchResumeToken` field:
...
For deployments enforcing <Ref url="authentication">{refs["authentication"].title}</Ref> and <Ref url="authorization">{refs["authorization"].title}</Ref>:

* To open a change stream against specific collection, applications
  must have privileges that grant <Ref url="changeStream">{refs["changeStream"].title}</Ref> and
  <Ref url="find">{refs["find"].title}</Ref> actions on the corresponding collection.
...
* To open a change stream on a single database, applications must have
  privileges that grant <Ref url="changeStream">{refs["changeStream"].title}</Ref> and
  <Ref url="find">{refs["find"].title}</Ref> actions on all non-`system` collections in the
  database.
...
* To open a change stream on an entire deployment, applications must
  have privileges that grant <Ref url="changeStream">{refs["changeStream"].title}</Ref> and
  <Ref url="find">{refs["find"].title}</Ref> actions on all non-`system` collections for all
...
For example, consider a 3-member <Ref url="replica set">{refs["replica set"].title}</Ref> with a change stream
cursor opened against the <Ref url="primary">{refs["primary"].title}</Ref>. If a client issues an insert
...
<Ref url="db.collection.watch-change-streams-pre-and-post-images-example">{refs["db.collection.watch-change-streams-pre-and-post-images-example"].title}</Ref>.
