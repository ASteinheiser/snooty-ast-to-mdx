...
version 1 (``).**Read Concern "majority" Enablement.**<Include href="/includes/extracts/changestream-rc-majority-4.2.rst">
  <Extract>
    Change streams are
    available regardless of the `` read concern
    support; that is, read concern `` support can be either
...
with the `` connection option or by listing the servers individually
...
a node with the correct read preference, it throws an exception.For more information, see Connection String URI Format.<span id="std-label-changeStreams-watch-deployment" />## Watch a Collection, Database, or DeploymentYou can open change streams against:<ListTable header-rows={{"$numberInt":"1"}} widths="15 85">
  ...
  (except `` collections, or any collections in the
  ``, ``, and `` databases).
  ...
  also the [``](https://www.mongodb.com/docs/mongodb-shell/#mongodb-binary-bin.mongosh) method
  ``.

  A database

  You can open a change stream cursor for a single database (excluding
  ``, ``, and `` database) to watch for changes to
  all its non-system collections.

  For the MongoDB driver method, refer to your driver
  documentation. See also the [``](https://www.mongodb.com/docs/mongodb-shell/#mongodb-binary-bin.mongosh) method
  ``.
  ...
  collections across all databases except for ``, ``, and
  ``.
...
number of opened change streams. For details see the maxPoolSize setting.### Sharded Cluster ConsiderationsWhen a change stream is opened on a sharded cluster:The `` creates individual change streams on **each
shard**. This behavior occurs regardless of whether the change stream
targets a particular shard key range.When the `` receives change stream results, it sorts and
filters those results. If needed, the `` also performs a
`` lookup.For best performance, limit the use of `` queries in
...
operation from the ``.The following example opens a change stream for a collection and
...
upper-right to set the language of the examples on this page.***<TabsSelector>
  ...
  <Tab tabid="kotlin-coroutine">
    ...
    that contains the `` collection. To learn more about completing these tasks, see the
  ...
  <Tab tabid="python">
    ...
    The Python examples below assume that you have [connected to a MongoDB replica set and have accessed a database](https://www.mongodb.com/docs/drivers/pymongo/) that contains an `` collection.
...
</Note>[^start-time]: You can specify a `` to open the cursor at a particular
...
upper-right to set the language of the examples on this page.***<Tabs tabset="drivers">
  ...
  <Tab tabid="java-sync">
    ...
    The `` list includes a single `` stage that
    ...
    Passing the `` to the `` method directs the
  ...
  the change stream event's `` field.
...
upper-right to set the language of the examples on this page.***<Tabs tabset="drivers">
  <Tab tabid="c">
    ...
    To return the most current majority-committed version of the updated
    document, pass the `` option with the `` value to the
    `` method.

    In the example below, all update operations notifications
    include a `` field that represents the *current*
  ...
  <Tab tabid="swift-async">
    ...
    the updated document, pass `` to the
    `` method.
  ...
  The `` field for an update event may be missing if one
...
resumeAfter or
startAfter when opening the cursor.<span id="std-label-change-stream-resume-after" />### `` for Change StreamsYou can resume a change stream after a specific event by passing a resume token
to `` when opening the cursor.See Resume Tokens for more information on the resume token.<Important>
  ...
  <Include href="/includes/extracts/changestream-invalid-events.rst">
    <Extract>
      You cannot use `` to resume a change stream after an
  ...
  <Tab tabid="c">
    ...
    In the example below, the `` option is appended to the stream options
    to recreate the stream after it has been destroyed. Passing the `` to
  ...
  <Tab tabid="csharp">
    ...
    In the example below, the `` is retrieved from the last change stream document
    and passed to the `` method as an option. Passing the ``
    to the `` method directs
  ...
  <Tab tabid="go">
    ...
    You can use [ChangeStreamOptions.SetResumeAfter](https://godoc.org/go.mongodb.org/mongo-driver/mongo/options#ChangeStreamOptions.SetResumeAfter)<!-- unsupported: named_reference -->
    ...
    specified in the resume token. The `` takes a
  ...
  <Tab tabid="java-sync">
    ...
    You can use the `` method to resume
    notifications after the operation specified in the resume
    token. The `` method takes a value that must
    resolve to a resume token, e.g. `` in the
    example below.
  ...
  <Tab tabid="kotlin-coroutine">
    ...
    token. The `` method takes a value that must
    resolve to a resume token, such as the `` variable in the
    example below.
  ...
  <Tab tabid="motor">
    ...
    You can use the `` modifier to resume
    notifications after the operation specified in the resume
    token. The `` modifier takes a value that must
    resolve to a resume token, e.g. `` in the
    example below.
  ...
  <Tab tabid="nodejs">
    ...
    You can use the `` option to resume
    notifications after the operation specified in the resume
    token. The `` option takes a value that must
    resolve to a resume token, e.g. `` in the
    example below.
...
</Tabs><span id="std-label-change-stream-start-after" />### `` for Change StreamsYou can start a new change stream after a specific event by passing a resume
token to `` when opening the cursor. Unlike
resumeAfter, `` can
...
by creating a new change stream.See Resume Tokens for more information on the resume token.<Important>
  ...
  The `` aggregation stage includes
  a resume token on the `` field.
  ...
  The `` command includes a resume token on the
  `` field.
  ...
  <Include href="/includes/note-decode-resume-tokens.rst">
    ...
    extension to [``](https://www.mongodb.com/docs/mongodb-shell/#mongodb-binary-bin.mongosh), that decodes hex-encoded
    resume tokens.

    You can install and run the [resumetoken](https://github.com/mongodb-labs/mongosh-snippets/tree/main/snippets/resumetoken)
    snippet from [``](https://www.mongodb.com/docs/mongodb-shell/#mongodb-binary-bin.mongosh):
    ...
    from the command line (without using [``](https://www.mongodb.com/docs/mongodb-shell/#mongodb-binary-bin.mongosh)) if ``
    ...
    [using snippets](https://www.mongodb.com/docs/mongodb-shell/snippets/working-with-snippets/#std-label-snip-using-snippets) in [``](https://www.mongodb.com/docs/mongodb-shell/#mongodb-binary-bin.mongosh).
  </Include>
</Tip><span id="std-label-change-event-resume-token" />#### Resume Tokens from Change EventsChange event notifications include a resume token on the `` field:```json
{
   "_id": {
...
```<span id="std-label-aggregate-resume-token" />#### Resume Tokens from ``When using the `` command, the ``
...
```<span id="std-label-getMore-resume-token" />#### Resume Tokens from ``The `` command also includes a resume token on the
`` field:```json
{
   "cursor": {
      "nextBatch": [],
...
must have privileges that grant `` and
`` actions on the corresponding collection.```javascript
{ resource: { db: <dbname>, collection: <collection> }, actions: [ "find", "changeStream" ] }
```To open a change stream on a single database, applications must have
privileges that grant `` and
`` actions on all non-`` collections in the
database.```javascript
{ resource: { db: <dbname>, collection: "" }, actions: [ "find", "changeStream" ] }
```To open a change stream on an entire deployment, applications must
have privileges that grant `` and
`` actions on all non-`` collections for all
...
`` and the ``.## CollationChange streams use `` binary comparisons unless an explicit collation is
provided.## Change Streams and Orphan Documents<Include href="/includes/change-streams-and-orphans.rst">
  ...
  Setting `` to `` uses the default retention
  ...
  the `` pre- and post-image retention time.
  ...
  do not cause a `` change event. A `` event
  always includes the post-image.

  To request a pre-image, you set `` to
  `` or `` in ``.
  To request a post-image, you set `` using the same
  method.

  Pre-images are written to the ``
  collection.

  The `` collection may become large. To
  ...
  <Important>
    ...
    <Include href="/includes/downgrade-for-pre-and-post-images.rst">
      ...
      the `` command before you can downgrade to an earlier
      MongoDB version.
    </Include>
  </Important>
