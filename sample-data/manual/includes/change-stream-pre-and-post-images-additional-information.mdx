import DowngradeForPreAndPostImages from './downgrade-for-pre-and-post-images.mdx';
import { refs } from '../references';

Pre- and post-images are not available for a <Ref url="change-stream-output">{refs["change-stream-output"].title}</Ref> if the images were:
...
* Removed after the pre- and post-image retention time set in
  ...
  * Setting `expireAfterSeconds` to `off` uses the default retention
    ...
    change stream events are removed from the <Ref url="oplog">{refs["oplog"].title}</Ref>.
...
* Limit the change stream event size to less than 16 mebibytes. To limit
  ...
  * Limit the document size to 8 megabytes. You can request pre- and
    post-images simultaneously in the <Ref url="change-stream-output">{refs["change-stream-output"].title}</Ref> if other change stream event fields like
...
* To request a pre-image, you set `fullDocumentBeforeChange` to
  `required` or `whenAvailable` in <Ref url="db.collection.watch">{refs["db.collection.watch"].title}</Ref>.
  To request a post-image, you set `fullDocument` using the same
  method.

* Pre-images are written to the <Ref url="config.system.preimages">{refs["config.system.preimages"].title}</Ref>
  collection.
...
<Important>
  ...
  <DowngradeForPreAndPostImages />
</Important>

<Seealso>
  * For change stream events and output, see
    <Ref url="change-stream-output">{refs["change-stream-output"].title}</Ref>.
